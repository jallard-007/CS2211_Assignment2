...
int main() {
  ...
  for (;;) {
    ...
    if (selection == 1) conversion(subMenuSelector('K', "kilometers", 'M', "miles"), 'K', "kilometer", "mile", 0.621371);
    else if (selection == 2) conversion(subMenuSelector('L', "liters", 'G', "gallons"), 'L', "liter", "gallon", 0.264172);
    else if (selection == 3) conversion(subMenuSelector('H', "hectares", 'A', "acres"), 'H', "hectare", "acre", 2.47105);
    else if (selection == 4) conversion(subMenuSelector('K', "kilograms", 'P', "pounds"), 'K', "kilogram", "pound", 2.20462);
    else return 1;
  }
}